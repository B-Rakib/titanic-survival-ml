...
def load_test_data(filepath: str) -> tuple:
    ...
    print("Données de test chargées: {} exemples".format(len(X_test)))
...
def print_metrics(metrics: Dict[str, float]) -> None:
    ...
    for metric_name, value in metrics.items():
        print("{:.<20} {:.4f}".format(metric_name.upper(), value))
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray,
    ...
    if output_path:
        ...
        print("Matrice de confusion sauvegardée: {}".format(output_path))
...
def plot_roc_curve(y_true: np.ndarray, y_proba: np.ndarray,
    ...
    if output_path:
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print("Courbe ROC sauvegardée: {}".format(output_path))

    plt.close()


def create_submission(passenger_ids: pd.Series, predictions: np.ndarray,
    ...
    print("\nFichier de soumission créé: {}".format(output_path))
    print("Prédictions: {} survivants sur {}".format(predictions.sum(), len(predictions)))


def evaluation_pipeline(model_path: str,
                        test_data_path: str,
                        submission_path: str,
                        plots_dir: str = "docs/plots") -> None:
    """
    ...
    print("Prédictions générées: {} exemples".format(len(predictions)))
